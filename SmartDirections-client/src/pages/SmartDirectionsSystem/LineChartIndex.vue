...
export default {
  ...
  computed: {
    currentCarCounter: function () {
      ...
      return this.carCounters.find(carCounter => carCounter.carCounterID === this.currentCarCounterID)
    },
    currentCarCounterSensor: function () {
      if (this.currentCarCounterID === null || this.currentCarCounterSensorID === null) return null
      return this.carCounterSensors.find(carCounterSensor => carCounterSensor.carCounterID === this.currentCarCounterID && carCounterSensor.carCounterSensorID === this.currentCarCounterSensorID)
    }
  },
  mounted () {
    const vm = this
    ...
    this.carCounterListener = this.$db.collection('carcounter').onSnapshot((carCounterSnapshot) => {
      carCounterSnapshot.docChanges().forEach(change => {
        if (change.doc.data().carCounterID !== null) {
          this.carCounterSensorListeners.push(
            this.$db.collection('carcounter').doc(change.doc.data().carCounterID).collection('carcountersensor').onSnapshot((carCounterSensorSnapshot) => {
              carCounterSensorSnapshot.docChanges().forEach(change => {
                if (change.doc.data().carCounterSensorID !== null) {
                  const index = this.carCounterSensors.findIndex(carCounterSensor => carCounterSensor.carCounterSensorID === change.doc.data().carCounterSensorID)
                  if (index < 0) {
                    this.carCounterSensors.push(change.doc.data())
                  } else {
                    this.carCounterSensors.splice(index, 1, change.doc.data())
                  }
          ...
          let index = this.carCounters.findIndex(carCounter => carCounter.carCounterID === change.doc.data().carCounterID)
          if (index < 0) {
            this.carCounters.push(change.doc.data())
          } else {
            this.carCounters.splice(index, 1, change.doc.data())
          }
          if (change.doc.data().geoPoint.Latitude !== null && change.doc.data().geoPoint.Longitude !== null) {
            index = this.markers.findIndex(marker => marker.carCounterID === change.doc.data().carCounterID)
            const marker = {
              carCounterID: change.doc.data().carCounterID,
              position: { lat: change.doc.data().geoPoint.Latitude, lng: change.doc.data().geoPoint.Longitude }
            }
            if (index < 0) {
    ...
    this.currentReadingListener = this.$db.collection('readings').onSnapshot((readingSnapshot) => {
      readingSnapshot.docChanges().forEach(change => {
        const index = this.currentReadings.findIndex(currentReading => currentReading.carCounterID === change.doc.data().carCounterID && currentReading.carCounterSensorID === change.doc.data().carCounterSensorID)
        if (index < 0) {
          ...
          date1.setSeconds(this.currentReadings[index].readingDateTime.seconds)
  ...
  methods: {
    setInfoWindowContent (carCounterID) {
      this.currentCarCounter = this.carCounters.find(carCounter => carCounter.carCounterID === carCounterID)
      this.currentCarCounterID = carCounterID
      if (this.currentOpenedMarker !== null && this.currentOpenedMarker.carCounterID === carCounterID) {
        let windowContent = ''
        const index = this.carCounters.findIndex(carCounter => carCounter.carCounterID === carCounterID)
        if (index >= 0) {
          windowContent = '<strong>' + this.carCounters[index].name + '</strong>'
          if (this.carCounters[index].urlImage !== null) {
            windowContent = windowContent + '<br><img height="100" width="100" src="' + this.carCounters[index].urlImage + '" alt="Meteo station image">'
          }
          windowContent = windowContent + '<br><button onclick="openDescription(\'' + carCounterID + '\')">Description</button>'
        }
        const carCounterSensors = this.carCounterSensors.filter(carCounterSensor => carCounterSensor.carCounterID === carCounterID)
        if (carCounterSensors.length > 0) {
          windowContent = windowContent + '<hr>'
        }
        carCounterSensors.forEach(carCounterSensor => {
          windowContent = windowContent + '<p><button onclick="openReadings(\'' + carCounterID + '\', \'' + carCounterSensor.carCounterSensorID + '\')">Readings</button> ' + carCounterSensor.sensor.name
    ...
    openReadings (carCounterID, carCounterSensorID) {
      ...
      this.readingsListener = this.$db.collection('readings')
        ...
        .onSnapshot((readingSnapshot) => {
          readingSnapshot.docChanges().forEach(change => {
            if (typeof change.doc.data().readingID !== 'undefined' && change.doc.data().readingID !== null) {
              const date = new Date(0)
              date.setSeconds(change.doc.data().readingDateTime.seconds)
              const localeDateString = date.toLocaleDateString()
              const localeTimeString = date.toLocaleTimeString()
              ...
              this.chartData.labels.push(localeDateString + '[' + localeTimeString + ']')
